...
(function ($) {
    ...
    var datatableInit = function () {
        ...
        var $table = $('#table_clients_commercial');
        // $table.dataTable({
        ...
        $('#table_clients_commercial tbody').on('click', 'button', function () {
            // var data = bis.row($(this).parents('tr')).data();
            // alert("" + data["id"]);
            // $('#id01').modal('show');
            window.location.href = '/commercial/client/'+bis.row($(this).parents('tr')).data()["id"];
            // $('#modalForm').getElementById("")
        });

        // Add the class for the search field is inside the table
        $("#table_clients_commercial_filter").addClass("col-md-10");
