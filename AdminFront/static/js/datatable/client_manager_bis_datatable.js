...
(function ($) {
    ...
    var datatableInit = function () {
        ...
        var $table = $('#table_clients_partener');
        // $table.dataTable({
        ...
        $('#table_clients_partener tbody').on('click', 'button', function () {
            // var data = bis.row($(this).parents('tr')).data();
            // alert("" + data["id"]);
            // $('#id01').modal('show');
            window.location.href = '/partener/client/'+bis.row($(this).parents('tr')).data()["id"];
            // $('#modalForm').getElementById("")
        });

        // Add the class for the search field is inside the table
        $("#table_clients_partener_filter").addClass("col-md-10");
